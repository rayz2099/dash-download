...
impl PeerHandler {
    ...
    async fn wait_for_bitfield(&self) {
        let wait = self.wait_for_any_notify(&self.on_bitfield_notify, || {
            self.state
                .peers
                .with_live(self.addr, |live| !live.bitfield.is_empty())
                .unwrap_or_default()
        });
        // 迅雷握手后经常不发 bitfield. 3s 还空就当全 have, 否则永远不 request.
        match tokio::time::timeout(Duration::from_secs(3), wait).await {
            ...
            Err(_) => {
                let empty = self
                    ...
                    .unwrap_or(true);
                if empty {
                    debug!("no bitfield after handshake, assume peer has all pieces");
                    let n = self.state.lengths.piece_bitfield_bytes();
                    let bf = BF::from_boxed_slice(vec![0xffu8; n].into_boxed_slice());
                    self.state.peers.update_bitfield(self.addr, bf);
                    self.on_bitfield_notify.notify_waiters();
                }
            }
