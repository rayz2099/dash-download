...
fn join_safe(base: &Path, rel: &Path) -> anyhow::Result<PathBuf> {
    if rel.components().any(|c| !matches!(c, Component::Normal(_))) {
        anyhow::bail!("path traversal in torrent file: {rel:?}");
    }
    Ok(base.join(rel))
}
...
impl TorrentStorage for FilesystemStorage {
    ...
    fn remove_file(&self, _file_id: usize, filename: &Path) -> anyhow::Result<()> {
        Ok(std::fs::remove_file(join_safe(&self.output_folder, filename)?)?)
    }
    ...
    fn remove_directory_if_empty(&self, path: &Path) -> anyhow::Result<()> {
        let path = join_safe(&self.output_folder, path)?;
        if !path.is_dir() {
    ...
    fn init(
        ...
        for file_details in metadata.file_infos.iter() {
            let full_path = join_safe(&self.output_folder, &file_details.relative_filename)?;
