...
### 新功能
- 自动嗅探网页已加载的视频，支持 X 与内嵌播放器，通过可拖动悬浮面板操作
- 可选清晰度，HLS/DASH 点播默认下载为 MP4，也可选择 MKV
- 后台添加任务，不抢桌面焦点；在应用内管理进度、暂停和恢复
- 桌面发布包内置 yt-dlp 与 FFmpeg

### 改进
- 短期复用视频解析结果，减少下载前重复解析清单的等待
- 提示暂不支持的直播与 DRM 视频，防止重复提交，失败后可重试

### 升级提示
- 桌面应用与 Chrome 扩展需要同时更新，重新加载扩展并刷新视频网页

## 1.2.8 - 2026-09-08

### 变更
- Chrome 扩展弹层只留接管开关, 默认开启
- 全站下载一律接管, 去掉 1MB 阈值和域名黑名单
- 全站权限改为必选, 不再额外申请网站访问

## 1.2.6 - 2026-09-08

### 修复
- 批量删除点确认立刻关弹窗, 单个失败不再卡住界面
- Chrome 扩展工具栏图标换新

## 1.2.5 - 2026-08-24

### 修复
- 发一个更新的 GitHub Release, 让 1.2.4 客户端能走应用内自动更新
