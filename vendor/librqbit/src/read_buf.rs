...
impl ReadBuf {
    ...
    // TCP 经常把 68 字节握手拆成多次 read; 单次 read 后 deserialize 会
    // NotEnoughData 直接失败, swarm 里看起来像 "有 seen 无 live".
    pub async fn read_handshake(
        &mut self,
        conn: &mut BoxAsyncReadVectored,
        timeout: Duration,
    ) -> Result<Handshake> {
        const HS_LEN: usize = 1 + 19 + 8 + 20 + 20;
        loop {
            if self.len > 0 {
                match Handshake::deserialize(&self.buf[..self.len]) {
                    ...
                    Err(MessageDeserializeError::NotEnoughData(..)) if self.len < HS_LEN => {}
            ...
            if n == 0 {
                return if self.len == 0 {
                    Err(Error::PeerDisconnectedReadingHandshake)
                } else {
                    Err(Error::DeserializeHandshake(
                        MessageDeserializeError::NotEnoughData(HS_LEN - self.len, None),
                    ))
                };
            }
            self.len += n;
