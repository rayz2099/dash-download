...
impl<H: PeerConnectionHandler> PeerConnection<H> {
    ...
    pub async fn manage_peer_incoming(
        ...
        .await?;
        incoming.writer.flush().await.map_err(Error::WriteHandshake)?;
    ...
    pub async fn manage_peer_outgoing(
        ...
        async move {
            ...
            // 迅雷/BitComet 默认 MSE. 明文能握手但不发 bitfield, 看起来永远 0 速.
            // MSE 失败则重连走明文: 同一 TCP 已经写过 Ya, 不能回退.
            let (mut read, mut write) = match crate::mse::initiate(read, write, &self.info_hash.0).await
            {
                Ok(rw) => {
                    debug!("mse ok");
                    rw
                ...
                Err(e) => {
                    debug!(error = format!("{e:#}"), "mse failed, reconnect plaintext");
                    let (_kind, r, w) = with_timeout(
                        "connecting",
                        connect_timeout,
                        self.connector.connect(self.addr),
                    )
            ...
            .await?;
            write.flush().await.map_err(Error::WriteHandshake)?;
