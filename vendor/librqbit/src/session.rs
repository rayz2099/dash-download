...
impl Session {
    ...
    async fn check_incoming_connection(
        ...
        let skeys: Vec<[u8; 20]> = self
            ...
            .map(|t| t.info_hash().0)
            .collect();
        let (mut reader, writer) = tokio::time::timeout(
            rwtimeout,
            crate::mse::maybe_incoming(reader, writer, &skeys),
        )
        .await
        .context("mse incoming timeout")?
        .context("mse incoming")?;
